...
flowchart TB
    U["用户 / 管理员"]
    F["前端管理控制台<br/>登录 / 文档 / 搜索 / 问答 / 任务 / 权限 / 查询日志 / 中英文"]
    N["Nginx 入口"]
...
依赖组件用途：
- `MySQL`：存储文档元数据、版本、权限策略、任务记录和查询日志
- `Redis`：承担缓存、状态协调和异步流程辅助
- `MinIO`：存储上传文档与解析前后的文件对象
- `Elasticsearch`：提供全文检索和关键词召回
- `Qdrant`：提供向量检索和语义召回

为什么需要全文检索、关键词召回：
- 业务文档里经常包含制度名、字段名、编号、产品名、错误码、合同号这类强关键词，单靠向量检索容易漏掉精确命中
- 全文检索更适合处理“用户问题里已经带明确术语”的场景，能更稳定地把目标片段先召回出来
- 关键词命中路径更容易解释和排障，适合知识库运营、相关性调优和权限治理后的结果核对
- 向量检索擅长语义相近召回，全文检索擅长精确术语召回，两者结合比单一路径更适合企业知识库问答

后端如何融合全文检索与向量检索：
- 后端搜索层会并行调用 `LexicalSearchClient` 和 `VectorSearchClient`
- 两路结果都会返回统一的 chunk 级命中项，字段包括 `chunkId`、`documentId`、标题路径、定位信息和分数
- 融合时按 `chunkId` 去重，如果同一个 chunk 同时被两路召回，就保留分数更高的一条
- 合并后再按分数倒序排序，并按 `topK` 截断为最终搜索结果
- 如果两路检索都不可用，后端还会退化到数据库中的 chunk 文本包含匹配，保证基础可用性
- 在权限链路上，融合后的结果还会继续经过文档级权限过滤，避免无权限文档进入搜索结果和 QA 引用

在 QA 链路中的使用方式：
- QA 会先对用户问题做改写，再复用同一套搜索融合结果作为候选证据
- 只有通过权限过滤的 chunk 才会进入上下文组装和最终回答生成
- 因此当前架构里，全文检索负责“精确找得到”，向量检索负责“语义找得全”，后端融合负责“结果可用且可控”
